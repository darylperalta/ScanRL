...
class MemoryBuffer(object):
    ...
    def memorize(self, state, action, reward, done, new_state, error=None):
        ...
        # print(state[:10])
        # print(new_state[:10])
        state_low = (state*255).astype(np.uint8)
        new_state_low = (new_state*255).astype(np.uint8)
        # print('after mem')
        # print(state_low[:10])
        # print(new_state_low[:10])

        # experience = (state, action, reward, done, new_state)
        experience = (state_low, action, reward, done, new_state_low)
    ...
    def sample_batch(self, batch_size):
        ...
        # print('prev')
        # print(s_batch[:10])
        # print(new_s_batch[:10])
        s_batch = (s_batch/255.0).astype(np.float64)
        new_s_batch = (new_s_batch/255.0).astype(np.float64)
        # print('after')
        # print(s_batch[:10])
        # print(new_s_batch[:10])
